...
#   * Go files generated by `wire` (:!:*wire_gen.go)
GO_FILES := $(shell $(GIT_LS_FILES) -- '*.go' ':!:*.pb.go' ':!:*wire_gen.go')
...
# WIRE_GEN_FILES contains all files not ignored by Git which are generated by `wire`.
WIRE_GEN_FILES := $(shell $(GIT_LS_FILES) -- '*wire_gen.go')
...
# wire-generate: generate dependency injection code using `wire`.
.PHONY: wire-generate
wire-generate: $(WIRE)
	$(WIRE) ./...

# lint-wire-generate: verify that files generated by `wire` are up to date.
.PHONY: lint-wire-generate
lint-wire-generate: wire-generate
	scripts/git-verify-no-diff.bash \
		$(WIRE_GEN_FILES)
