...
func TestValidateName(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: fmt.Sprintf("%s/%s/%s", store, CollectionID, purchaseID), want: nil},
		{name: "", want: ErrNameInvalidFormat},
		{name: purchaseID, want: ErrNameInvalidFormat},
		{name: fmt.Sprintf("%s/%s/%s/%s", users.CollectionID, storeID, CollectionID, purchaseID), want: ErrNameInvalidFormat},
...
func TestParent(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name:       store + "/" + CollectionID + "/" + purchaseID,
			wantParent: store,
			wantErr:    nil,
		},
		{
			name:       users.CollectionID + "/" + storeID + "/" + CollectionID + "/" + purchaseID,
			wantParent: "",
